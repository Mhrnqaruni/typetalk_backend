...
### Fixes Applied — Review Round 8

1. Google canceled-but-unexpired subscriptions were losing paid access immediately even when `current_period_end` was still in the future.
- Did I confirm it: Yes. Before the fix, I reproduced the inspector finding against the live app flow: `POST /v1/billing/google-play/verify-subscription` for a Google subscription in provider state `CANCELED` with a future `current_period_end` returned `subscription.status = "canceled"` but `subscription.entitlement_code = "expired"`, and `GET /v1/entitlements/current` returned inactive free access instead of preserving paid access through the paid-through period.
- What I fixed: I added shared subscription-access helpers in `src/modules/billing/subscription-access.ts` so entitlement decisions use effective access rules instead of raw status checks. The unified entitlement recomputation in `src/modules/entitlements/service.ts` now keeps `CANCELED` subscriptions entitling while their `currentPeriodEnd` is still in the future, and `src/modules/billing/service.ts` now uses the same rule when deciding whether an organization already has active paid access.
- How I verified: `npx vitest run test/integration/billing.google-play.test.ts test/integration/billing.google-rtdn.test.ts`, `npm run build`, `npm run test`, and a direct in-process repro all passed. The direct repro now returns `subscription.status = "canceled"` with `subscription.entitlement_code = "pro_active"` on verify, and `GET /v1/entitlements/current` now returns `status = "active"` with `code = "pro_active"` until the period end is reached.

2. Claimed Google acknowledgment retries could get stranded forever after a crash because the claim path cleared the retry timestamp and the retry job no longer saw the row as due.
- Did I confirm it: Yes. Before the fix, I reproduced the failure by forcing an acknowledgment error, making the token due, manually claiming it through the repository, and then running the retry job. The token moved to `acknowledgmentStatus = "PENDING"` with `acknowledgmentNextRetryAt = null`, and the retry job reported `scanned = 0` and `processed = 0`, leaving the token stuck.
- What I fixed: I changed `claimPurchaseTokenAcknowledgmentRetry` in `src/modules/billing/repository.ts` so a claim writes a reclaim deadline instead of nulling `acknowledgmentNextRetryAt`, and I updated `src/modules/billing/google-play-support.ts` to compute that reclaim deadline from the existing stale-lock timeout before claiming the row. This keeps the row invisible to concurrent workers during the claim window but makes it automatically reclaimable after a crash.
- How I verified: the new reclaim regression test in `test/integration/billing.google-play.test.ts` passed, `npm run build` passed, `npm run test` passed, `npm run billing:webhooks:retry` passed, and a direct in-process repro now shows the claimed row retaining a non-null `acknowledgmentNextRetryAt`, the retry job processing `1` row after the reclaim window is forced due, and the token finishing in `ACKNOWLEDGED` state with `acknowledgedAt` populated.
...
- No open functional Phase 4 billing issues were identified after the Review Round 8 verification rerun.
