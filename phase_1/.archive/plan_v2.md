...
2. Define the Phase 1 data model in Prisma.
   - What to do: add Phase 1 tables, enums, constraints, and indexes to `prisma/schema.prisma` for `users`, `auth_identities`, `email_challenges`, `sessions`, `organizations`, `organization_members`, `devices`, and minimum `security_events` support required for auth flows. The `sessions` model must be the stable family anchor for refresh rotation, must keep the current `refresh_token_hash`, and must add `reauthenticated_at` so recent re-auth can be enforced explicitly.
   - Which files are affected: `prisma/schema.prisma`.
   - Expected outcome / how to verify: the schema captures the locked fields and rules from `final_plan.md`, including unique Google identity keys, one active challenge per email and purpose, hashed refresh tokens, a stable session anchor for rotation/reuse detection, `reauthenticated_at` for recent re-auth enforcement, personal organizations, device ownership by user, and durable auth-related security events.
   - Potential risks: missing unique constraints, incorrect nullability, or skipping the stable session-family and `reauthenticated_at` design will make replay detection or recent re-auth impossible to implement safely later.
...
11. Implement authenticated request context plus refresh and logout flows.
   ...
   - Expected outcome / how to verify: authenticated routes can access the current user/session context; refresh succeeds only when the `session_id` exists and the submitted secret matches the current stored hash; successful refresh rotates the secret in place and returns a new access token plus new refresh token; logout revokes the anchored session.
   - Potential risks: if the refresh token does not carry a stable session identifier, replay detection becomes ambiguous; if rotation creates parallel valid secrets, token theft detection becomes unreliable.
...
13. Implement user and session-management endpoints.
   ...
   - Expected outcome / how to verify: authenticated users can read and update profile data, list their active sessions with `limit` and `cursor`, revoke a selected session safely, and soft-delete their own account in a way that preserves the row for auditability but blocks future active access.
   - Potential risks: missing ownership checks can allow session revocation across accounts; a soft delete that does not revoke sessions or exclude the user from active lookups leaves deleted accounts effectively alive.
